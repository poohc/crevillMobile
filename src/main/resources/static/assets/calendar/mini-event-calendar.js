(function( $ ) {
	...
	`;
	var weekDaysFromSunday = '<div>일</div><div>월</div><div>화</div><div>수</div><div>목</div><div>금</div><div>토</div>';
	var weekDaysFromMonday = '<div>M</div><div>T</div><div>W</div><div>T</div><div>F</div><div>S</div><div>S</div>';
	var shortMonths = ["1월", "2월", "3월", "4월", "5월", "6월", "7월","8월", "9월", "10월", "11월", "12월"];
		...
		function populateCalendar(month, year, onInit) {
			...
			while (ldate.getMonth() === month) {
		        ...
		        if(eventIndex != -1){
     			...
     			var bufferDays = 31 - miniCalendar.find(".a-date").length;
